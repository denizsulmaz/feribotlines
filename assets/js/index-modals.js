...
var picker = new Pikaday({
  ...
  onSelect: function (date) {
    pickerDate = date;
    if (datePickerWay === 1) {
      datePickerElement.parents().eq(2).find(".going-day").text(date.getUTCDate() + 1);
      ...
      // Dönüş tarihi
      datePickerElement.parents().eq(2).find(".return-day").text(date.getUTCDate() + 1);
      ...
      initForm.find("#goingDate").val(date.getUTCDate() + 1 + "." + curDate + "." + date.getUTCFullYear());
      ...
      datePickerElement.parents().eq(2).find(".return-day-name").text(weekdays[date.getDay()]);
      var curDate = date.getMonth() + 1;
      initForm.find("#returnDate").val(date.getUTCDate() + 1 + "." + curDate + "." + date.getUTCFullYear());
    }
...
$(document).ready(function () {
  ...
  $(".date-holder-left").find(".going-day").text(getDate.getUTCDate());
  ...
  $(".date-holder-right").find(".return-day").text(getDate.getUTCDate());
  ...
  $(".dynamic-form-form-submit").click(function (e) {
    ...
    if (activeForm === "ferry") {
      if ($("#ferryForm .routes .from .route").text(fromText) && $("#ferryForm .routes .to .route").text(toText)) {
        alert("Bir veya birden fazla liman seçmediniz.");
        return false
      } else {
        showPreloader();
        $(this).parent().submit();
